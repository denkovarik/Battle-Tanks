...
bool Edelweiss::retreat(int &x, int &y, PositionData status, MapData map)
{
        ...
        for(int j = -1; j < 2; j++)
	{
		...
		if((blocked(map, status, myX + j, myY + i, true) || threat.dis >= dis) )
		{
		    if(canHitAnything(map, status, myX + j, myY + i))
		    	safe = false;
		}
	    }
...
bool Edelweiss::makePlan(MapData map, PositionData status, vector<spot> &vantagePoints)
{
    int x = status.game_x;
    int y = status.game_y;
    double dist;
    ...
    enemyTank threat = tankList.front();
	...
	for(int j = -1; j < 2; j++)
	{
	    ...
	    if(!blocked(map, status, x + j, y + i, true) && dist > threat.dis && !inRadar(x + j, y + i, threat) && canHitAnything(map,status, x + j, y + i))
	    {
		newSpot.x = x + j;
		newSpot.y = y + i;
		inRangeOutRadar.push_back(newSpot);
            ...
            else if(!blocked(map, status, x + j, y + i, true) && dist > threat.dis && !inRadar(x + j, y + i, threat) && !canHitAnything(map,status, x + j, y + i))
            {
                newSpot.x = x + j;
                newSpot.y = y + i;
                outRangeOutRadar.push_back(newSpot);
            }
            ...
            else if(!blocked(map, status, x + j, y + i, true) && dist > threat.dis && inRadar(x + j, y + i, threat) && !canHitAnything(map,status, x + j, y + i))
            {
                newSpot.x = x + j;
                newSpot.y = y + i;
                outRangeInRadar.push_back(newSpot);
            }
	}
    }
    ...
    else if(!outRangeOutRadar.empty())
    {
	cout << "OutRangeOutRadar\n";
        vantagePoints = outRangeOutRadar;
        return true;
    }

    else if(!outRangeInRadar.empty())
    {
        vantagePoints = outRangeInRadar;
        return true;
    }
...
int Edelweiss::spendAP(MapData map, PositionData status)
{
    ...
    //Fight of Flight
    if(canHitAnything(map, status, status.game_x, status.game_y))
    {
