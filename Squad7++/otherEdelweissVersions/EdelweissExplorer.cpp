...
{
    int minRadarX = status.game_x - 4, minRadarY =  status.game_y - 4 ;
    int maxRadarX = status.game_x + 4, maxRadarY =  status.game_y + 4 ;
    cout << "Map size: " << map.map.size() << endl;
    ...
    for(int x= 0; x < map.width; ++x)
    {
	    for(int y = 0; y < map.height; ++y)
	    {
		    ...
		    if(map.map[x + y * map.width] == 0 && x >= minRadarX && x <= maxRadarX 
